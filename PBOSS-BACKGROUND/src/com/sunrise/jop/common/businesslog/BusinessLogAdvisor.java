...
public class BusinessLogAdvisor extends AbstractAdvisor {
	...
	static {
		methodsMap.put(METHOD_CREATE, null);
		methodsMap.put(METHOD_UPDATE, null);
		methodsMap.put(METHOD_REMOVE, null);
		methodsMap.put(METHOD_REMOVEBYVO, null);
		methodsMap.put(METHOD_REMOVEBYPK, null);
	}

	private boolean checkArgs(Method method, Object[] args) {
		boolean requireLog = false;

		if (methodsMap.containsKey(method.getName().substring(0, 8)) || methodsMap.containsKey(method.getName().substring(0, 12)))
			requireLog = true;
	...
	private int countObjectToRemove(Object object, Method method,
		...
		try {
			...
			BaseVO baseVO = (BaseVO) commonControl.doFindByPk(pk); // 为提高效率，以后改为
			// count方法
			if (baseVO != null)
				return 1;
			else
