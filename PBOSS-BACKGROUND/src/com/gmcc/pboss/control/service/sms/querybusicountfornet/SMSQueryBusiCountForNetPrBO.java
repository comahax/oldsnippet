...
public class SMSQueryBusiCountForNetPrBO extends AbstractControlBean implements SMSQueryBusiCountForNetPr {
	private static Logger logger = Logger.getLogger(SMSQueryBusiCountForNetPrBO.class);
	...
	public String doQueryBusiCountForNet(String mobile,String cityid,String smscontent)
		...
		try {
			...
			if(!"qdcx".equalsIgnoreCase(content[0])){
			...
			if(length!=2 && length!=3){
				logger.info("=======length!=2 && length!=3======1.检查短信格式===开始=======");
				Map<String,String> map = new HashMap<String,String>();
				Waitchange  waitchangeBO = (WaitchangeBO) BOFactory.build(WaitchangeBO.class,user);
				waitchangeBO.doInsertForCj("CJ_016", map, mobile);
				return doRecordSmshisAndReturnSuccVal (mobile, cityid, smscontent,"CJ_016",new Short("1")).toString();
			}else if(length==2){
				logger.info("=====length==2=======1.检查短信格式===开始=======");
				date1 = content[1];
				...
				logger.info("=====length==3========1.检查短信格式===开始=======");
				date1 = content[1];
				date2 = content[2];
				if(!PublicUtils.checkDateTime(2, date1, "", "", "")){
					logger.info("=======date1======1.检查短信格式===开始=======");
					Map<String,String> map = new HashMap<String,String>();
					Waitchange  waitchangeBO = (WaitchangeBO) BOFactory.build(WaitchangeBO.class,user);
					waitchangeBO.doInsertForCj("CJ_016", map, mobile);
					return doRecordSmshisAndReturnSuccVal (mobile, cityid, smscontent,"CJ_016",new Short("1")).toString();
				...
				if(days<0){
					logger.info("=====days<0========1.检查短信格式===开始=======");
					Map<String,String> map = new HashMap<String,String>();
					Waitchange  waitchangeBO = (WaitchangeBO) BOFactory.build(WaitchangeBO.class,user);
					waitchangeBO.doInsertForCj("CJ_016", map, mobile);
					return doRecordSmshisAndReturnSuccVal (mobile, cityid, smscontent,"CJ_016",new Short("1")).toString();
				...
				if(days>30){
					logger.info("=======days>30======1.检查短信格式===开始=======");
					Map<String,String> map = new HashMap<String,String>();
					Waitchange  waitchangeBO = (WaitchangeBO) BOFactory.build(WaitchangeBO.class,user);
			...
			//2.检查上行号码是否为店员捆绑号码
			logger.info("=============2.检查上行号码是否为店员捆绑号码===开始=======");
			...
			param.getQueryConditions().put("WAYID", employeevo.getWayid());
