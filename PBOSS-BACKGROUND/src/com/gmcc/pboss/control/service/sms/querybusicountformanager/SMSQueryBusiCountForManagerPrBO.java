...
public class SMSQueryBusiCountForManagerPrBO extends AbstractControlBean implements SMSQueryBusiCountForManagerPr {
	private static Logger logger = Logger.getLogger(SMSQueryBusiCountForManagerPrBO.class);
	...
	public String doQueryBusiCountForManager(String mobile,String cityid,String smscontent)
		...
		try {
			...
			//date0为qdjlcx,date1为网点编码,date2为起始日期，date3为截止日期
			String wayid = "";
			String date1 = "";
			String date2 = "";
...
//				qdjlcx#渠道编号#yyyymmdd
				...
				logger.info("======length==4=======检查短信格式===开始=======");
//				qdjlcx#渠道编号#yyyymmdd#yyyymmdd
				wayid = content[1];
				date1 = content[2];
				date2=content[3];
				udate1 = PublicUtils.strToSqlDate(date1, "yyyyMMdd");
				...
				if(days>30){
					logger.info("=======days>30======检查短信格式===开始=======");
					Map<String,String> map = new HashMap<String,String>();
					Waitchange  waitchangeBO = (WaitchangeBO) BOFactory.build(WaitchangeBO.class,user);
					waitchangeBO.doInsertForCj("CJ_024", map, mobile);
					return doRecordSmshisAndReturnSuccVal (mobile, cityid, smscontent,"CJ_024",new Short("1")).toString();
				}
			}
			
			//2.检查上行号码是否为渠道经理公务机号码
			logger.info("=============2.检查上行号码是否为渠道经理公务机号码===开始=======");
			...
			if(null!=dp&&dp.getDatas().size()>0){
				...
				Map<String,String> map = new HashMap<String,String>();
				Waitchange  waitchangeBO = (WaitchangeBO) BOFactory.build(WaitchangeBO.class,user);
				waitchangeBO.doInsertForCj("CJ_010", map, mobile);
				return doRecordSmshisAndReturnSuccVal(mobile,cityid,smscontent,"CJ_010",new Short("1")).toString();
			}
			
			//2.5  偷笑ing...
			//检查渠道编码是否为该渠道经理管辖范围内网点
			logger.info("=============检查渠道编码是否为该渠道经理管辖范围内网点===开始=======");
	...
	public static void main(String []args) throws Exception {
		...
		System.out.println("j算m-".matches(strCheck));
	}
}
