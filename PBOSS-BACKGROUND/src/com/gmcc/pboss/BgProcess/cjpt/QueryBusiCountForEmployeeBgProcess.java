 ...
 *
 */
public class QueryBusiCountForEmployeeBgProcess extends BgCommonBase {
	private static Logger logger = Logger.getLogger(QueryBusiCountForEmployeeBgProcess.class);
		
	public static void main(String[] args) throws Exception{

		QueryBusiCountForEmployeeBgProcess action = new QueryBusiCountForEmployeeBgProcess();
		...
		action.setMyProfilePath("/cjpt/queryBusiCountForEmployee.properties");
		// 初始化
		action.init(args);
		action.queryBusiCountForEmployee(user);
		
	}
	
	private void queryBusiCountForEmployee(User user) throws Exception {
		try {			
			...
			ctiparam.set_ne_smstype("3");//店员查询
				...
				if (null==dp || dp.getDatas().size()==0){
					...
					for (Iterator it = dp.getDatas().iterator(); it.hasNext();) {
						try{
							...
							bo = (SMSQueryBusiCountForEmployeePr) BOFactory.build(
									SMSQueryBusiCountForEmployeePrBO.class, user);
