...
function setExcelOutPage(url) {
	...
	var setPageUrl = contextPath + "/common/excelout/selectpage.jsp";
...
function getCurrBillCyc()
{
	...
	var currBillCyc = "";
	if((month+"").length == 1){
		currBillCyc = year + "0" + (month+1) + "00";
	}
	else{
		currBillCyc = year + "" + (month+1) + "00";
	} 
	return currBillCyc;
}

function getLastBillCyc()
{
	var myDate = new Date();
	var year = myDate.getFullYear();
	var month = myDate.getMonth();
	var billCyc="";
	if((month+"").length == 1){
		billCyc = year + "0" + (month+1) + "00";
	}
	else{
		billCyc = year + "" + (month+1) + "00";
	}
	
	var thisBillCyc =  parseInt(billCyc);
	var lastBillCyc;
	if (thisBillCyc % 10000 == 100) {// 1月份帐期，则上一月为12月
		lastBillCyc = thisBillCyc - 8900;
	} else {
		lastBillCyc = thisBillCyc - 100;
	}
	return lastBillCyc;
}