...
public class Code2NameCom extends Component {
	...
	public boolean end(Writer writer, String body) {
		if (definition != null && definition.trim().length() > 0 && code != null && code.trim().length() > 0) {
			...
			} else if( !dbFlagValue.equals(DBConstant.DB_FLAG_GDIB) ) {
				dbFlagValue = CityMappingUtil.getCity(user.getCityid()) + dbFlag; // XXIB/XXBILL
			}
			try {

				if (dbFlagValue == null) {
					dbFlagValue = DBConstant.DB_FLAG_COMMON;
				}
				String[] codeValues;
