...
<body>
	...
	<div class="divspan">
                  ...
                  <tr>
                    ...
                    <td colspan="3">
                        <select id='wayType' name='wayType'>
                            ...
                            <option value="UNPB" selected>创新联盟</option>
                        </select>  	</td>
                  </tr>
                  <tr>
		...
		<s:if test="%{result.data.size > 0}">
				...
				<c:forEach items="${result.data}" var="bbcRewardTotal" varStatus="i">
                  <tr>
              <td width="25%" bgcolor="#FFFFFF" class="tableTitle">${bbcRewardTotal.rewardtypeName}</td>
				<td bgcolor="#FFFFFF"><span class="tableMark"><fmt:formatNumber value='${bbcRewardTotal.paymoney}' pattern='${fmtNumber}'/></span>元</td>
                  </tr>
				<c:set var="sumReward" value ="${sumReward+bbcRewardTotal.paymoney}"/>
				</c:forEach>
			</s:if>
          </table>
