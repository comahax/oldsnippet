...
public class MagRegisternewQueryParamProcessor extends DefaultHqlQueryProcessor
		implements QueryParameterProcessor {

	@Override
	public String getHql(QueryParameter parameter) {
		// TODO Auto-generated method stub
		MagRegisternewQueryParameter p = (MagRegisternewQueryParameter)parameter;
		...
		//hql.append(" r.mobile,r.brand,r.opnid,o.opnname,to_char(r.oprtime,'yyyy-MM-dd hh24:mi:ss') as oprtime)");
		...
		else{//如果网点编码为空，则限定查询当前经理下属网点；如果未限定，查询范围会放大
			//hql.append(" and w.wayid in ( select ww.wayid from com.gmcc.pboss.biz.info.node.model.Way ww where ww.waymagcode=:waymagcode)");
			hql.append(" and w.wayid in (select wayid from CH_PW_WAY where waymagcode=:waymagcode)");
		}
		//店员-工号
	...
	public void process(Query query, QueryParameter parameter) {
		...
		if(p.getTimeFrom()!=null && p.getTimeTo()!=null){
			...
			query.setDate("start", p.getTimeFrom());
			Date end = p.getTimeTo();
