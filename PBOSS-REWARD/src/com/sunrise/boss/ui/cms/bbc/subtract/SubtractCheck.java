...
public class SubtractCheck extends BaseCheckFormat {

	public SubtractCheck(){
	...
	public void checkLine(String line, int rowCount, User user) throws Exception {
		...
		if(StringUtils.isEmpty(content[0].trim())){
			throw new BusinessException("","一次性支付酬金业务编码不能为空");
		}
		...
		for (int i = 0; i < content.length-1; i++) {
			...
			case 0:
				if (!CheckUtil.checkString(content[i], 18, true)) {
					throw new Exception("[一次性支付酬金业务编码]不能大于18位");
				}
				break;
			case 1:
