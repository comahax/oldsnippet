...
public class BatchTaskBean extends BaseBatchTaskBean {
	MicroareaDelegate delegate;
	public BatchTaskBean() {
		try {
			delegate = new MicroareaDelegate();
			batchName = "微区域管理";
			setWriteLog(true);
		} catch (Exception ex) {
	...
	protected String doStart() {
		return "微区域编码|服务销售中心编码|微区域名称|微区域类型|法人代表|计费用行政区域代码|行政区划编码|办公地点经度|办公地点纬度" + "\r\n";
	}
	/**
	 * 处理一条记录
	 */
	public ResultVO processLine(String line, int rowCount) {
		...
		MicroareaVO vo = new MicroareaVO();
		try {
			MicroareaVO oldvo = delegate.doFindByPk(items[0], user);
			if (oldvo == null) {// 新增
			vo.setMacode(items[0]);
			vo.setSvccode(items[1]);
			vo.setManame(items[2]);
			vo.setMatype(new Short(items[3]));
			vo.setAgent(items[4]);
			...
			delegate.doCreate(vo, user);
				...
				if (items[1] != null && !items[1].equals("")) {
					oldvo.setSvccode(items[1]);
				}
				if (items[2] != null && !items[2].equals("")) {
					oldvo.setManame(items[2]);
				}
				if (items[3] != null && !items[3].equals("")) {
					oldvo.setMatype(new Short(items[3]));
				}
