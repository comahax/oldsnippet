 ...
 *
 */ 
public class BatchSTRTWayTaskBean extends BaseBatchTaskBean {
	...
	protected String doStart() {
		return "序号 | 渠道编码 | 渠道名称 | 上级渠道编码 | 合作方 | 地市公司 | 分公司 | 服务销售中心 | 微区域 | 星级 | 合作层级 | 行政区划 |地理经度 | 地理纬度 | 详细地址 | 负责人姓名 | 负责人联系电话 | 负责人电子邮箱 | 业务联系人姓名 | 业务联系人联系电话 | 业务联系人电子邮箱 | 合同编码 | 合同名称 | 签署合同时间 | 合同到期日 | 法人代表 | 营业执照编号 | 经营区域类型编码 | 经营范围 | 帐户标识 | 开户银行 | 银行帐号 | 开户账号名称 | 开户人身份证号码 | 送货地址 | 收货联系人 | 收货联系号码 | 收货人证件号码 | 营业执照有效期 | 保证金下限 | 开户日期|生效时间|失效时间|" + "\r\n";
	}
...
//		去空格
		...
		try {
			...
			if (wayVO1 == null) {// 新增
				...
				wayVO.setTaxtype("".equals(items[8].trim())? null : new Short(items[8]));
				wayVO.setMainlayer("".equals(items[9].trim())? null : new Short(items[9]));
			...
			if (waycompactVO1 ==null) {
				...
				waycompactVO.setBegintime("".equals(items[22].trim())? null : new  java.sql.Date(format.parse(items[22]).getTime()));
				waycompactVO.setSigntime("".equals(items[22].trim())? null : new  java.sql.Date(format.parse(items[22]).getTime()));
				waycompactVO.setEndtime("".equals(items[23].trim())? null : new  java.sql.Date(format.parse(items[23]).getTime()));
				waycompactVO.setPrincipal(items[24]);
				waycompactVO.setLicenceno(items[25]);
				waycompactVO.setRunareatype("".equals(items[26].trim())? null : new Short(items[26]));
				waycompactVO.setCopbound(items[27]);
				waycompactVO.setBail("".equals(items[37].trim())? null : new Double(items[37]));
				waycompactVO.setBegintime("".equals(items[39].trim())? null : new  java.sql.Date(format.parse(items[39]).getTime()));
				waycompactVO.setEndtime("".equals(items[40].trim())? null : new  java.sql.Date(format.parse(items[40]).getTime()));
				...
				if (items[22] == null || "".equals(items[22])) {
					...
					waycompactVO.setBegintime(new  java.sql.Date(format.parse(items[22]).getTime()));
					waycompactVO.setSigntime(new  java.sql.Date(format.parse(items[22]).getTime()));
				}
				...
				if (items[23] == null || "".equals(items[23])) {
					...
					waycompactVO.setEndtime(new  java.sql.Date(format.parse(items[23]).getTime()));
...
//			帐户信息
			...
			cooperatorvo1 = coopdelegate.doFindByPk(cooperatorvo.getCooperauid(), user);
			...
			cooperatorvo.setDistrictid(SessionFactoryRouter.conversionCityid2Num(wayVO.getCityid()));
			if (cooperatorvo1 == null) {
				...
				cooperatorvo.setLicvalidate("".equals(items[36].trim())? null : new  java.sql.Date(format.parse(items[36]).getTime()));
				cooperatorvo.setIntime("".equals(items[38].trim())? null : new  java.sql.Date(format.parse(items[38]).getTime()));
				...
				if (items[36] == null || "".equals(items[36])) {
					...
					cooperatorvo.setLicvalidate(new  java.sql.Date(format.parse(items[36]).getTime()));
				if (items[38] == null || "".equals(items[38])) {
					...
					cooperatorvo.setIntime(new java.sql.Date(format.parse(items[38]).getTime()));
...
//			重组VO
			 ...
			 wayVO = (WayVO)utils.doSaveAudit(wayVO, "CH_PW_WAY", "CH_PW_STRBWAY", newwayfield,waypk, "CH_PW_SOTYWAY_AUDIT",user);
			 cooperatorvo = (CooperatorVO)utils.doSaveAudit(cooperatorvo, "CH_DST_COOPERATOR", "CH_PW_STRBWAY",cooperatorfield,cooperaupk,  "CH_PW_SOTYWAY_AUDIT",user);
			 wayaccountVO = (WayaccountVO)utils.doSaveAudit(wayaccountVO, "CH_PW_WAYACCOUNT", "CH_PW_STRBWAY", wayaccountfield,wayaccountpk, "CH_PW_SOTYWAY_AUDIT",user);
	...
	public String showInfo(ResultVO resultVO, String[] items, int rowCount) {
		...
		resultStr.append(items[38]).append(COMPART)
		.append(items[39]).append(COMPART)
		.append(items[40]).append(COMPART)
		.append("增加");
