...
public class BatchServcentCheck extends BaseCheckFormat {
	public BatchServcentCheck() {
		super();
	}
	...
	public void checkLine(String line, int rowCount, User user)
		...
		if (items[0] == null || "".equals(items[0].trim())
				|| items[0].length() > 14)
			throw new Exception("[服务销售中心编码]不能为空，也不能大于14位！");
		if (delegate.doFindByPk(items[0], user) == null) {// 新增

			if (items[1].length() > 14)
				throw new Exception("[分公司编码]过长！必须不大于14位");
			if (items[2].length() > 64)
				throw new Exception("[服务销售中心名称]过长！必须不大于64位");
			try { // 检查items[3]是否为数字
				Double temp = Double.valueOf(items[3]);
			} catch (Exception e) {
				e.printStackTrace();
				throw new Exception("[服务销售中心类型]类型不对(" + items[3]
						+ "),应该是数字字符串!");
			}
			if (items[3].getBytes().length > 3)
				throw new Exception("[服务销售中心类型]过长！必须不大于3位");
