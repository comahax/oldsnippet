...
public class OperationXmlBuilder extends AbstractXmlBuilder {
	...
	public OperationXmlBuilder(User user) {
	...
	public String getChildXml(XTreeNode xtreeNode) throws Exception {
		...
		// int curr = StringUtils.isEmpty(opntype) ? 4 : (new Integer(opntype)
		// .intValue());
		if ("operation".equals(nodeType)) {
			...
			if(element!=null && element.getValue()!=null){
				...
				for(int index=0;index<list.size();index++){
					...
					OperationVO vo=delegate.doFindByPk(values[0], user);
	...
	private void doCheckoperation(XTreeNode xtreeNode, StringBuffer buffer, OperationVO vo, XTreeNode node) throws Exception {
	...
	protected boolean isLeaf(XTreeNode xtreeNode) throws Exception {
		...
		OperationVO operationVO = delegate.doFindByPk(xtreeNode.getId(), user);
