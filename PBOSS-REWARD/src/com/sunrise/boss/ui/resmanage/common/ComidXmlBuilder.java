...
public class ComidXmlBuilder extends AbstractXmlBuilder {
	...
	public String getChildXml(XTreeNode xTreeNode) throws Exception {
		...
		if (PARENTTYPE_TOP.equals(parentType)) {
			...
			if (tree != null && tree.haveChilds()) {
				for (int i = 0; i < tree.childsLength(); i++) {
					...
					if (map.containsKey("comtype")
							&& !((List) map.get("comtype")).contains(childTree
									.getValue())) {
						continue;
					}
					...
					if (list != null && list.size() > 0) {
						...
						node = createNode(comtype, xTreeNode.getParentId(),
								comtypeName, PARNETTYPE_COMTYPE, xTreeNode);
			...
			if (tree != null && tree.getChild(parentId) != null) {
				leafs = tree.getChild(parentId).getLeafsByCityid(cityId,
						filterFlag);
				if (leafs != null && leafs.size() > 0) {
					for (Iterator it = leafs.iterator(); it.hasNext();) {
						...
						text = vo.getComname();
						node = createNode(id, xTreeNode.getParentId(), text,
								PARNETTYPE_COMID, xTreeNode);
						buf.append(getNodeString(node));
					}
				}
			}
	...
	protected String getSrcString(XTreeNode xtreeNode) throws Exception {
		...
		url = com.sunrise.pub.tools.StringUtils.escapeForXML(url);
	...
	// protected String buildAll(XTreeNode xTreeNode,String cityId) throws
	// Exception {
	// XTreeNode node;
	//
	// StringBuffer buf = new StringBuffer("");
	// buf.append(writeFirstNode(" ", xTreeNode.getParentId(),
	// ComidTreeHelper.comclassidnames[0],
	// ComidTreeHelper.PARNETTYPE_COMID, xTreeNode));
	// for (int i = 1; i < ComidTreeHelper.comclassids.length; i++) {
	// node = createNode(ComidTreeHelper.comclassids[i], xTreeNode
	// .getParentId(), ComidTreeHelper.comclassidnames[i],
	// ComidTreeHelper.PARENTTYPE_COMCLASSID, xTreeNode);
	// buf.append(getNodeString(node));
	// }
	//
	// return buf.toString();
	...
	protected String buildSpecTree(XTreeNode xTreeNode, Map map, String cityId)
			throws Exception {
		XTreeNode node;
		...
		for (int i = 0; i < comclassids.length; i++) {
			...
			if (ComidTreeHelper.comclassidMap.containsKey(comclassid)) {
				...
				if (checkTree(tree,comtypes,cityId)) {
					comclassidName = (String) ComidTreeHelper.comclassidMap
							.get(comclassids[i]);
					node = createNode(comclassid, xTreeNode.getParentId(),
							comclassidName, PARENTTYPE_COMCLASSID, xTreeNode);
					buf.append(getNodeString(node));
				}
	...
	private XTreeNode createNode(String id, String parentId, String text,
			String type, XTreeNode xTreeNode) {
	...
	private String writeFirstNode(String parentId, XTreeNode xTreeNode)
			throws Exception {
		XTreeNode node = createNode(" ", parentId, "空值", PARNETTYPE_COMID,
				xTreeNode);
