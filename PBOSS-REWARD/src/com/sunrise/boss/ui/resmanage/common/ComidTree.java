	...
	protected void addChildAndLeaf(ComVO vo, User user) {
		...
		if (haveChilds()) {
			...
			if (flag) {
				...
				childTree.setName(ComidTreeHelper.getComtypeName(vo
						.getComtype(), user));
			...
			ComidTree childTree = new ComidTree();
			childTree.setValue(vo.getComtype().toString());
			childTree.setName(ComidTreeHelper.getComtypeName(vo.getComtype(),
					user));
			childTree.addLeaf(vo);
			childs.add(childTree);
		}
...
//		if (haveChilds()){
//			for (Iterator it = childs.iterator(); it.hasNext();) {
//				List leafList = ((ComidTree)it.next()).getLeafsByCityid(cityId, filterFlag);
...
//	}
	...
	private void initTree() {
		try {
			...
			ComListVO listvo = new ComListVO();
			CommonDelegate dt = new CommonDelegate(ComVO.class);
			listvo.set_ne_comclassid(this.value);
			listvo.set_orderby("comtype");
			listvo.set_pagesize("0");
			listvo.set_ne_comstate("1");
			DataPackage dp = dt.doQuery(listvo, user);
