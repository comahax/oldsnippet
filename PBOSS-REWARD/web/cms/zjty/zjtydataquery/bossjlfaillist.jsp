...
<head>
    ...
    <script language="JavaScript" type="text/JavaScript">
        function ev_check() {
         ...
         function doQuery() {
        	var url = contextPath + '/cms/zjty/zjtydataquery.do?CMD=Bossjlfail';
        	formList.action = url;
		    formList.submit();
        }
    </script>
...
<html:form action="/cms/zjty/zjtydataquery.do?CMD=LIST" styleId="formList" method="post" onsubmit="return ev_check();">
    ...
    <table class="top_table">
    	<tr>
    		<td>
    			<bean:message bundle="zjtydataquery" key="titleListBossjlfail"/>
   			</td>
    	</tr>
    </table>
    </div>
      ...
      <div class="table_LongTable">
        <table class="table_style" ID="Table2">
            <tr class="table_style_head">
                ...
                <td>
                    <bean:message bundle="zjtydataquery" key="wrapfee"/>
                </td>
                 <td>
                    <bean:message bundle="zjtydataquery" key="noncyc"/>
                </td>
                <td>
                    <bean:message bundle="zjtydataquery" key="rewardtype"/>
                </td>
                <td>
                    <bean:message bundle="zjtydataquery" key="calcmonth"/>
                </td>
                <td>
                    <bean:message bundle="zjtydataquery" key="calcflag"/>
                </td>
                 <td>
                    <bean:message bundle="zjtydataquery" key="calcremark"/>
                </td>
                <td>
                    <bean:message bundle="zjtydataquery" key="createtime"/>
                </td>
                <td>
                    <bean:message bundle="zjtydataquery" key="wadttime"/>
                </td>
                <td>
                    <bean:message bundle="zjtydataquery" key="updatetime"/>
                </td>
                <td>
            ...
            <c:forEach var="item" items="${requestScope.LIST.datas}">
                 ...
                 <tr class="table_style_content" align="center">
                     ...
                     <td><c:out value="${item.calcflag}"/></td>
                     <td><c:out value="${item.calcremark}"/></td>
                     <td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${item.createtime}"/></td>
                     <td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${item.wadttime}"/></td>
                     <td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${item.updatetime}"/></td>
                     <td><s:Code2Name code="${item.adtflag}" definition="#CH_ADTFLAG" /></td>
