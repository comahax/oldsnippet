...
<html>
	<head>
		...
		<%
			...
			String ID_2 = "CH_ZJTY_REWARD||CH_ZJTY_PROVINCIAL||CH_ZJTY_CIVIC"; //市级酬金管理令牌
			String ID_3 ="";
	...
	<body>
		...
		<script language="javascript">
		  addmenuleft();	
		     addmenu('<%=contextPath%>/cms/zjty/operation.do?CMD=EDIT<%=urlParams%>','业务基本信息'); 
		     <s:RewardPurChk controlid="<%=ID_1%>">
		     	addmenu('<%=contextPath%>/cms/zjty/stdrewardbj.do?CMD=SHOW<%=urlParams%>', '计件酬金上限设置');
		     </s:RewardPurChk>
		     <s:RewardPurChk controlid="<%=ID_2%>">
		    	addmenu('<%=contextPath%>/cms/zjty/stdrewardbj.do?CMD=SHOWCITY<%=urlParams%>', '市公司计件酬金上限设置');
		     </s:RewardPurChk>
			addmenuright();
		  addmenumore();
		</script>
			...
			<div class="iframewindow" id="message">
				<iframe framespacing="0" frameborder="NO" name="IFRM_MAIN" id="IFRM_MAIN" class="IFRM_MAIN_on"
					src="<%=contextPath%>/cms/zjty/operation.do?CMD=EDIT<%=urlParams%>" height="100%" scrolling="auto"></iframe>
			</div>
		</div>
	</body>
</html>
