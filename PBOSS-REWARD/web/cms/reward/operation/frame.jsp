...
<html>
	...
	<body>
		...
		<div class="table_container">
		  ...
		  addmenuleft();	
		     addmenu('<%=contextPath%>/cms/operation.do?CMD=EDIT<%=urlParams%>','业务基本信息'); 
		     addmenu('<%=contextPath%>/cms/reward/stdrewardbj.do?CMD=SHOW<%=urlParams%>', '计件酬金上限设置');
		     addmenu('<%=contextPath%>/cms/reward/stdrewardbj.do?CMD=SHOWCITY<%=urlParams%>', '市公司计件酬金上限设置');
		     addmenu('<%=contextPath%>/cms/reward/rule2.do?CMD=LIST<%=urlParams%>', '可用有效性规则查询');
			 <s:RewardPurChk controlid="<%=ID_1%>" >
			 	addmenu('<%=contextPath%>/cms/rewardrulequery.do?CMD=SHOW', '业务酬金规则查询');
			 </s:RewardPurChk>
			addmenuright();
		  addmenumore();
		</script>
