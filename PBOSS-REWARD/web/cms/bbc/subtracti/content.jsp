...
<html>
<head>
    <title><bean:message bundle="subtract" key="titleUpdate1"/></title>
    ...
    <script language="JavaScript">
        function ev_checkval() {
            ...
            addfield('intvopnid', '<bean:message bundle="subtract" key="intvopnid"/>', 'c', false, '18');
...
<html:form action="/cms/bbc/subtracti.do?CMD=SAVE" styleId="formItem" method="post">
    ...
    <html:hidden property="_sk_intvopnid"/>
    <c:set var="edtState" scope="request" value="${!empty param.CMD and (param.CMD eq 'EDIT' or param.CMD eq 'NEW')}"/>
    <c:set var="updateState" scope="request" value="${!empty param.CMD and (param.CMD eq 'EDIT')}"/>
    <c:set var="form" scope="request" value="${requestScope['/cms/bbc/SubtracitForm']}"/>
    ...
    <table class="top_table">
    	<tr>
    		<td>
    			<bean:message bundle="subtract" key="titleUpdate1"/>
   			</td>
    	</tr>
    </table>
    </div>
    ...
    <div class="table_div">
        <table class="form_table">
        	<tr>
        	 ...
        	 <tr>
                <td width="20%" align="right" class="form_table_right"><div class="field-require"><bean:message bundle="subtract" key="intvopnid"/>:</div></td>
                <td width="30%" align="left" class="form_table_left">
                    <c:choose>
                        <c:when test="${edtState}">
                        	<c:if test="${!updateState}">
                        		<html:text styleClass="form_input_1x" property="intvopnid"/>
                        		<font color=red>&nbsp;*</font>
                        	</c:if>
                        	<c:if test="${updateState}">
                        		<html:text styleClass="form_input_1x" property="intvopnid" disabled="true"/>
							<font color=red>&nbsp;*</font>
                        	</c:if>         
                        </c:when>
                        <c:otherwise>
            ...
            <tr>
                <td width="100%" class="form_table_center">
                    <input type="button" onmouseover="buttonover(this);" onmouseout="buttonout(this);"
                           ...
                           onclick="doSave('/cms/bbc/subtracti.do?CMD=SAVE')"/>
                    <input type="button" onmouseover="buttonover(this);" onmouseout="buttonout(this);"
                           name="btnReturn" onfocus="buttonover(this)" onblur="buttonout(this)"
                           value="<bean:message bundle="public" key="button_return"/>" class="close"
                           onclick="doReturn('/cms/bbc/subtracti.do?CMD=LIST')">  
                </td>
            </tr>
        </table>
    </div>
