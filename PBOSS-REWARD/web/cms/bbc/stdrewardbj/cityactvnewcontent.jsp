...
function ev_checkval() {
	...
	if (formItem.basicflag.value == 'true') {
		...
		if(parseFloat(formItem.max_rewardstd.value) < parseFloat(formItem.rewardstd_basic.value)){
			alert('该基本酬金上限不能大于省公司上限标准['+formItem.max_rewardstd.value+']，请重新设置');
			return false;
		}
	}
...
function ev_checkvalEccourage() {
	...
	if(parseFloat(formItem.max_city_rewardstd.value) < parseFloat(formItem.rewardstd_encourage.value)){
		alert('该奖励酬金上限不能大于省公司上限标准['+formItem.max_city_rewardstd.value+']，请重新设置');
		return false;
	}	
	return checkval(window);
}
					...
					<table class="form_table">
						...
						<tr>
							...
							<c:if test="${form.acctype_basic eq '1'}">
								<font color=red>(省公司酬金上限:<c:out value='${form.max_rewardstd}'/>元)</font>
							</c:if>
							<c:if test="${form.acctype_basic eq '2'}">
								<font color=red>(省公司酬金上限:<c:out value='${form.max_rewardstd*100}'/>%)</font>
							</c:if>
							</td>
						</tr>
					</table>
					...
					<table class="form_table">
						...
						<tr>
							...
							<td width="30%" class="form_table_left" colspan=3>
								...
								<font color=red>*&nbsp;(省公司上限:<c:out value='${form.max_city_rewardstd}'/>元)</font>
							</td>
						</tr>
					</table>
