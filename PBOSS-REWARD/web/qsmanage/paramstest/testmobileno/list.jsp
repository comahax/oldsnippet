...
<html>
	<head>
		<title><bean:message bundle="testmobileno" key="titleList" />
		</title>
		<script language="JavaScript">
        function ev_check() {
            addfield('_se_mobileno', '<bean:message bundle="testmobileno" key="mobileno"/>', 'c', true, 32);
            addfield('_ne_state', '<bean:message bundle="testmobileno" key="state"/>', 'i', true, 2);
	...
	<body onload="loadforiframe()">
		<div class="table_container">
			<html:form action="/qsmanage/paramstest/testmobileno.do?CMD=LIST" styleId="formList" method="post" onsubmit="return ev_check();">
						...
						<tr>
							...
							<c:choose>
								<c:when test="${edtState}">
									<html:text styleClass="form_input_1x" property="_se_mobileno" disabled="true" />
								</c:when>
								<c:otherwise>
									<html:text styleClass="form_input_1x" property="_se_mobileno" />
								</c:otherwise>
							</c:choose>
						</td>
							<td width="126" height="20" align="right" class="form_table_right">
					...
					<table class="table_button_list">
						<tr>
							...
							<input type="button" name="btnNew" class="add" onmouseover="buttonover(this);" onmouseout="buttonout(this);"
									...
									onClick="doNew('/qsmanage/paramstest/testmobileno.do')">
								</s:PurChk>
							<s:PurChk controlid="<%=ID_2%>">
								<input type="button" name="btnDelete" class="delete" onmouseover="buttonover(this);"
									onmouseout="buttonout(this);" onfocus="buttonover(this)" onblur="buttonout(this)"
									value="<bean:message bundle="public" key="button_delete"/>" onClick="doDelete('/qsmanage/paramstest/testmobileno.do')">
							</s:PurChk>
					...
					<div class="table_LongTable">
						<table class="table_style" ID="Table2">
							<tr class="table_style_head">
								...
								<td>
									<bean:message bundle="testmobileno" key="mobileno" /> 
									<s:OrderImg form="/qsmanage/paramstest/testmobileno/TestMobilenoForm" field="mobileno" />	
								</td>
								<td>
									<bean:message bundle="testmobileno" key="state" />
									<s:OrderImg form="/qsmanage/paramstest/testmobileno/TestMobilenoForm" field="state" />
								</td>
								
							</tr>
							<c:forEach var="item" items="${requestScope.LIST.datas}">
							<c:url value="/qsmanage/paramstest/testmobileno.do?CMD=EDIT" var="urlContent">
										<c:param name="PK" value="${item.mobileno}" />
									</c:url>
									<tr class="table_style_content" align="center">
										<td>
											<input type="checkbox" name="_selectitem" value="<c:out value='${item.mobileno}'/>" onclick="checkOne();"
												class="table_checkbox">
										</td>
										<td>
											<a href='<c:out value="${urlContent}"/>'><c:out value="${item.mobileno}" /></a>
										</td>
										<td>
