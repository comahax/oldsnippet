...
<html:html>
<head>
	<title><bean:message bundle="WoffRule" key="titleUpdate" /></title>
      	...
      	function doComplete(){
      		...
      		if(isnew == 'NEW'){
      			if(confirm("本菜单尚未进行参数设置，是否结束设置？")){
      				formItem.action = contextPath + "/qsmanage/paramsmanage/acctgroup/list.jsp";
      				formItem.submit();
      			}
      		}else{
      			formItem.action = contextPath + "/qsmanage/paramsmanage/acctgroup/list.jsp";
...
<body>
	<div class="table_container">
		<html:form action="/qsmanage/paramsmanage/acctgroup.do?CMD=WOFFRULE"
					...
					<tr>
						<td>
							<input type="button" name="btnNew" class="add"
								...
								onClick="doSave('/qsmanage/paramsmanage/acctgroup.do?CMD=WOFFRULEMORE')">
							<c:choose>
								<c:when test="${edtState}">
									<input type="button" onmouseover="buttonover(this);"
										...
										class="submit"
										onclick="doSave('/qsmanage/paramsmanage/acctgroup.do?CMD=WOFFRULE')" />
								</c:when>
								<c:otherwise>
									<input type="button" onmouseover="buttonover(this);"
										onmouseout="buttonout(this);" name="btnSave"
