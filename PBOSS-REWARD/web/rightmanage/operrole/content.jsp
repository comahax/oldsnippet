...
<html:html>
<head>
	<title><bean:message bundle="operrole" key="titleUpdate" /></title>
	<script language="JavaScript">
        function ev_checkval() {
			addfield('itemid', '<bean:message bundle="operright" key="itemid"/>', 'c', false, 32);
...
<div class="table_container">
	<html:form action="/operrole.do?CMD=SAVE" styleId="formItem" method="post">
		...
		<table class="top_table" border=0>
					<tr>
						<td>
							<bean:message bundle="operrole" key="titleUpdate" />
						</td>
					</tr>
		</table>
		</div>
				...
				<tr>
					...
					<td width="30%" align="left" class="form_table_left">
						<c:choose>
							...
							<c:when test="${edtState}">
								<html:text styleClass="form_input_1x" property="createdate2" disabled="true" onclick="this.value=selectDate()"	/>
								<bean:message bundle="fee" key="redStar" />
							</c:when>
							<c:otherwise>
				...
				<tr>
					<td>
								...
								<c:when test="${newState or edtState}">
									<s:PurChk controlid="<%=ID_1%>">
										<input type="button" onmouseover="buttonover(this);"
										  	 ...
										  	 class="submit"
										  	 onclick="doSave('/operrole.do?CMD=SAVE')" />
									</s:PurChk>
								</c:when>
							</c:choose>		
						...
						<input type="button" onmouseover="buttonover(this);"
						       ...
						       class="close"
							   onclick="doReturn('/operrole.do?CMD=LIST')"/>
						</s:PurChk>
