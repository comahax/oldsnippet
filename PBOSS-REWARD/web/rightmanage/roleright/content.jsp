...
<html:html>
<head>
	<title><bean:message bundle="roleright" key="titleUpdate" /></title>
...
<div class="table_container">
	<html:form action="/roleright.do?CMD=SAVE" styleId="formItem" method="post">
		...
		<table class="top_table" border=0>
					<tr>
						<td>
							<bean:message bundle="roleright" key="titleUpdate" />
						</td>
					</tr>
		</table>
		</div>
					...
					<td width="30%" align="left" class="form_table_left">
						<c:choose>
							<c:when test="${newState}">
								<html:text styleClass="form_input_1x" property="itemid" onblur="this.value=ignoreSpaces(this.value);"/>
								<bean:message bundle="fee" key="redStar" />
							</c:when>
							<c:when test="${edtState}">
								<html:text styleClass="form_input_1x" property="itemid" disabled="true" onblur="this.value=ignoreSpaces(this.value);"/>
								<bean:message bundle="fee" key="redStar" />
							</c:when>
							<c:otherwise>
								<html:text styleClass="form_input_1x" property="itemid" disabled="true" onblur="this.value=ignoreSpaces(this.value);"/>
				...
				<tr>
					<td>
								...
								<c:when test="${newState or edtState}">
									<s:PurChk controlid="<%=ID_1%>">
										<input type="button" onmouseover="buttonover(this);"
										  	 ...
										  	 class="submit"
										  	 onclick="doSave('/roleright.do?CMD=SAVE')" />
									</s:PurChk>
								</c:when>
							</c:choose>		
						...
						<input type="button" onmouseover="buttonover(this);"
						       ...
						       class="close"
							   onclick="doReturn('/roleright.do?CMD=LIST')"/>
						</s:PurChk>
