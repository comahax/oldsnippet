...
String extend1 = (String) request.getAttribute("EXTEND1");
...
<html>
	...
	<div class="table_container">			
		<div class="table_div">
			...
			<table width="95%" border="0" cellspacing="0" cellpadding="0" class="top_table">
                <tr> 
		...
		<div class="table_div">
			<table class="table_style">
					<tr class="table_style_content">
						<c:if test="${requestScope.typename1!=''}">
						...
						<td align=left>
							 <c:choose>
                       			 <c:when test="${requestScope.typename1!=''}">
									<%= type1 %> + <%= type2 %>
                        		</c:when>
                        		<c:otherwise>
	                        		已选业务项
					...
					<tr class="table_style_content">
						...
						<c:if test="${requestScope.typename1!=''}">
						<td align=left>
							<input type=text class="form_input_1x" name="prefix_type1" value="" onkeyup="waitforchange('/commons/rewardopr.do?CMD=change', 's2')">
						</td>
						</c:if>
						<td align=left>
							<input style="display:none" type="text" name="areacode" value="750" />
							<input type=text class="form_input_1x" name="prefix_type2" value="" onkeyup="waitforchange('/commons/rewardopr.do?CMD=change', 's2')">
						</td>
					...
					<tr class="table_style_content">
						<c:if test="${requestScope.typename1!=''}">
						<td align=left>
								<select size=10 id=s1 name=s1 class="multi"  >
									<option value=""></option>
									<c:forEach var="item" items="${LIST_TYPE1}" varStatus="id">
								      	<option value=<c:out value="${item.id}"/> <c:out value="${item.name}"/>><c:out value="${item.id}"/> <c:out value="${item.name}"/></option>
								    </c:forEach>
								</select>
						</td>
						</c:if>
						<td align=left>
								<select size=10 id=s2 name=s2 class="multi" onclick="ff()" onmouseout = "cc()" >
									<option value=""></option>
					...
					<tr class="table_style_content">
						<td colspan="4" align="center">
								<input  type=button  value="确定" onclick="doConfirm()" class="button_2">
								<input  type=button  value="关闭"  onclick="window.close()" class="button_2">
						</td>
					</tr>
					<tr class="table_style_content">
...
function doConfirm() {
	...
	for (var i=0; i<elm3.length; i++) {
		var tempStr=elm3.options[i].value;
		str = str + tempStr + '<%=symbol_tail%>';
	}
	var len = str.length;
	if (len == 0) {
		str = '';
