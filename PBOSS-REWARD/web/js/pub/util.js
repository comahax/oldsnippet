...
//get combo values
function getComboValues(definition) {
	var originalValue ='';
	...
	var arg = new Array();
	var strUrl = contextPath + "/commons/combineinput.do?CMD=select&definition=" + definition + "&originalStr=" + originalValue;
...
function getMultiValues(definition, splitChar) {
	var originalValue = event.srcElement.value;
	
	var arg = new Array();
	var strUrl = contextPath + "/commons/multiselect.do?CMD=SHOW" +
				"&DEFINITION=" + definition + 
				"&SPLITCHAR=" + splitChar + 
				"&CODESTR=" + originalValue;
	var ret = window.showModalDialog(strUrl, arg, "dialogWidth:720px; dialogHeight:330px; status:no; resizable:no;");
	if (ret == null || ret == '') {
		ret = originalValue;
	} else if (ret == 'BLANK') {
		ret = '';
