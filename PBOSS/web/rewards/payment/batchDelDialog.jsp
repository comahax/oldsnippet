...
<script language="JavaScript" type="text/JavaScript">
	 ...
	 (function(){
		 ...
		 trIndex = setAddTr(send.payee, "收款单位名称： ",trIndex);
		 trIndex = setAddTr(send.ltype, "酬金大类： ",trIndex);
		 trIndex = setAddTr(send.stype, "酬金小类： ",trIndex);
		 trIndex = setAddTr(send.paymonth, "付款月份： ",trIndex);
		 trIndex = setAddTr(send.batch, "批次： ",trIndex);
		 trIndex = setAddTr(send.pubpri, "对公/对私： ",trIndex);
		 trIndex = setAddTr(send.calcmonth, "结算月份： ",trIndex);
		 ...
		 if(cmdType == "batchSend"){
			 totalpre = "发送";
			 if(switchflag=="open"){
				 explaintxt = "当前只允许发送审核标识为已审核且未发送过的数据";
			 }else{
				 explaintxt = "当前只允许发送未发送过的数据";
			 }
