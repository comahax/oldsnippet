...
function AjaxAnywhere() {
    ...
    this.delayToDropPreviousRequest = 4000;  //used with isInSubmiting ,when it's true
...
AjaxAnywhere.prototype.substituteFormSubmitFunction = function() {
    ...
    form.submit = function() {
    	...
    	if ( typeof ajax !='object' || !ajax.isFormSubmitByAjax() ){
    		ajax.findForm().submit_old();
    	}else if(event && event.srcElement && ajax.isSubmitableButton( event.srcElement ) && !ajax.isAjaxButton(event.srcElement.name)){
    		ajax.findForm().submit_old();
    	}else if(event == null){
    		ajax.submitAJAX();  
    		//if other element invoke ajax, it should be work!
    	}else{
    		//ajax.submitAJAX();
    		//if true, ajaxanywhere invoke a private method named "handleSubmitButtonClick" when the button clicked! and will be submit twice! by Linli 2009-11-11
    	}
    };
