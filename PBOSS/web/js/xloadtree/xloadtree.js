...
function WebFXLoadTreeItem(sText, sXmlSrc, sAction, eParent, sIcon, sOpenIcon, bChecked) {//Add checkbox Modify by Canigar 
	// call super
	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(sText, sAction, eParent, sIcon, sOpenIcon, bChecked); //Add checkbox Modify by Canigar 
...
function _startLoadXmlTree(sSrc, jsNode) {
	...
	// 调用ajax到sSrc获取jsNode下所有子节点的xml信息
	xmlHttp.open("GET", sSrc, true);	// async
	xmlHttp.onreadystatechange = function () {
		if (xmlHttp.readyState == 4) {
			// 将xmlHttp.responseXML(即调用该sSrc返回的xml信息)挂到jsNode该节点下
...
function _xmlTreeToJsTree(oNode, check) {//Add checkbox modify by Canigar
	// retreive attributes
	...
	//because the first time should judge check property!
	if (typeof(check)=='undefined'){
	  	check = oNode.getAttribute("check");
	}
	// create jsNode
	var jsNode;
	if (src != null && src != "")
		jsNode = new WebFXLoadTreeItem(text, src, action, parent, icon, openIcon, check);
	else
		jsNode = new WebFXTreeItem(text, action, parent, icon, openIcon, check);
	...
	for (var i = 0; i < l; i++) {
		if (cs[i].tagName == "tree")
			jsNode.add( _xmlTreeToJsTree(cs[i],check), true );
	}
	...
	if( oXmlDoc == null || oXmlDoc.documentElement == null) {		
		...
		for (var i = 0; i < l; i++) {
			if (cs[i].tagName == "tree") {
				bAnyChildren = true;
				bIndent = true;
				jsParentNode.add( _xmlTreeToJsTree(cs[i] , jsParentNode._checked), true);
			}
		}
...
function refresh(uid){
  for(var i = 0; i < webFXTreeHandler.idCounter;i++){
    var item = webFXTreeHandler.all[(webFXTreeHandler.idPrefix + i)];
    if(item==null)continue;
    var action = item.action;
    if(action==null)continue;
    var iUid = action.substring(action.indexOf("(")+1,action.indexOf(")"));
    if(iUid==("'" + uid + "'")){
      item.reload();
      break;
    }
