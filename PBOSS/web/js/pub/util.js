...
function selectDateYYYYMM() {
	...
	/*var arg = new Object();
	var strTime = event.srcElement.value;
	var retDate = window.showModalDialog(contextPath + "/js/pub/selectmon.html", strTime, "dialogWidth=240px;dialogHeight=170px;status:no;scroll=no;resizable:yes;");
	if (!retDate){
		retDate = "";
	}
	return retDate ;*/
...
function setExcelOutPage(url) {
	...
	var setPageUrl = contextPath + "/common/export/selectpage.jsp";
	var hWnd = window.showModalDialog(setPageUrl, arg, "dialogWidth:600px; dialogHeight:250px; status:no;resizable:yes;");
	if (hWnd != null && hWnd != "") {
		var infoPage = hWnd.split("|");
		eval("document.all('param.startindex').value = infoPage[0]");
		eval("document.all('param.endindex').value = infoPage[1]");
