...
function date_comparebyid(element1,element2,errorMessage) {
			var beginTime = document.getElementById(element1).value;
			var endTime = document.getElementById(element2).value;
			if(beginTime != "" && endTime != ""){
				var dt_begin = (beginTime.split("."))[0];
				var dt_end = (endTime.split("."))[0];
				if(dt_begin > dt_end) {
...
//  ckdoc??????????window????
//----------------------------------------------------------------------------------------------------------------------
function checkval(ckdoc) //ckdoc must is window object
{
  // check fields var
  var i;
  var iscorrect = true;
  ...
  for(i=0;i<fields.length;i++)
  {
    ...
    if(elvalue == null || elvalue == "")
    {
      ...
      {
        alertstr +=  '<span class=\'errorkey\' style=\'font-size:12px;\'><span style=\'color:#F00; font-size:12px;\'>[' + fields[i].cnname + ']</span>:' + msgInputNotNull + '</span>';
        iscorrect = false;
        continue;
      }
    }
    ...
    {
		if(isitemcorrect == 13){
			alertstr += '<span class=\'errorkey\'><span style=\'color:#F00; font-size:12px;\'>[' + fields[i].cnname + ']</span>' + errorstr[isitemcorrect] + fields[i].len1 +msgPotinInfo + msgStringOutRangeInfo +'</span>';
		}
		else if(isitemcorrect == 15){
			alertstr += '<span class=\'errorkey\'><span style=\'color:#F00; font-size:12px;\'>[' + fields[i].cnname + ']</span>' + errorstr[isitemcorrect] + fields[i].maxval + '</span>';
		}
		else if(isitemcorrect == 16){
			alertstr += '<span class=\'errorkey\'><span style=\'color:#F00; font-size:12px;\'>[' + fields[i].cnname + ']</span>' + errorstr[isitemcorrect] + fields[i].minval + '</span>';
		}
		else if(isitemcorrect == 22){
			alertstr += '<span class=\'errorkey\'><span style=\'color:#F00; font-size:12px;\'>[' + fields[i].cnname + ']</span>' + errorstr[isitemcorrect] + fields[i].len1 +msgPotinInfo +'</span>';
		}
		else if(isitemcorrect == 23){
			alertstr += '<span class=\'errorkey\'><span style=\'color:#F00; font-size:12px;\'>[' + fields[i].cnname + ']</span>' + errorstr[isitemcorrect] + fields[i].len2 +msgPotinInfo +'</span>';
		}
		else if(isitemcorrect == 24){
			alertstr += '<span class=\'errorkey\'><span style=\'color:#F00; font-size:12px;\'>[' + fields[i].cnname + ']</span>' + errorstr[isitemcorrect] + fields[i].len1 +msgPotinInfo +'</span>';
		}
		else{
			alertstr += '<span class=\'errorkey\'><span style=\'color:#F00; font-size:12px;\'>[' + fields[i].cnname + ']</span>' + errorstr[isitemcorrect] + '</span>';
		}
...
function errorMessageShow (str) {
	//alert(str);
	errorobj.innerHTML = "<div id=\"msgDiv\" style=\"width:98%; Height:22px; padding-top:6px; text-align:left;\">"+str+"</div>";
	}
function addfield(name,cnname,type,isnull,len1,len2,defaultval,minval,maxval)
{
  ...
  //????????????????????
  //----------------------------------------------------------------------------------------------------------------------
  if((type=="i"||type=="f")&&!minval) minval = 0;
  
  if(type=="d"&&!minval) minval = -9223372036854775808;
...
function validatedate( strValue )
{
  ...
  if(!objRegExp.test(strValue))
  {
    ...
    if (strValue.length==8) {
    	intYear = strValue.subStrnig(0,4);
    	intMonth = strValue.substring(4,6);
        intDay = strValue.substring(6,8);
      
    } else {
    	var arrayDate=strValue.split(strValue.substring(4,5));
    	intYear = arrayDate[0];
