	...
	var weebox = function(content,options) {
		...
		this.initContent = function(content) {
			...
			if (!self.options.showClose) {
				self.dh.find('.dialog-close').hide();
			}	
		...
		this.focus = function() {
			$(window).focus();
			var okBtn = self.dh.find('.dialog-ok');
			var cancelBtn = self.dh.find('.dialog-cancel');
			if (self.options.focus) {
				...
				self.dh.find('.dialog-content').focus();
			}
		}
		//在弹窗内查找元素
	...
	var weeboxs = function() {		
		...
		$(document).click(function(event) {
			...
			if (event.keyCode == 9){
				...
				if (arrweebox.length>0) {
					if (target.calssName=='dialog-ok'){
						self.dh.find('.dialog-cancel').focus();
					}else if (target.calssName=='dialog-cancel'){
						self.dh.find('.dialog-ok').focus();
					}else {
						$(window).focus();
					}
					return false;//返回false来防止触发浏览器的默认行为
				}//if
			}//if
		});
	}
