...
<head>
    ...
    <script language="JavaScript" type="text/JavaScript">
        function ev_check() {
            addfield('param._se_prodid', '<s:text name="prodid"/>', 'c', true, '32');
            return checkval(window);
        }
        
        function doNew(str) {
   		 	formList.action ="<%=contextPath%>/promotion/pquantity_new.do?pk=" + str;
   			formList.submit();
		}
		...
		function doDelete(str, str1){
			...
			formList.action ="<%=contextPath%>/promotion/pquantity_delete.do?pk=" + str + "&pk1=" + str1;
			formList.submit();
			}else{
			formList.action ="<%=contextPath%>/promotion/pquantity_list.do?pk=" + str + "&pk1=" + str1;
			formList.submit();
			}
		}
...
<s:form action="pquantity_list.do" key="formList" cssStyle="formList" theme="simple" onsubmit="return ev_check();">
	...
	<div class="table_div">
        <table class="table_normal">
            <tr>
                <td align="center"><s:text name="prodid"/>:</td>
                <td align="left">
                   <p:smpBrand name="param._se_prodid" mode="of" cssStyle="style_input" />
                </td>
            </tr>
        </table>
    </div>
    ...
    <div class="table_div">
        ...
        <table class="table_style">
            <tr class="table_style_head">
                ...
                <td>
                    <j:orderByImg href="javascript:doOrderby('prodid')"><s:text name="prodid"/></j:orderByImg>                 
                </td>
                <td>
                    <j:orderByImg href="javascript:doOrderby('incratio')"><s:text name="incratio"/></j:orderByImg>                 
                </td>
            </tr>
            <s:iterator value="dp.datas">
                 <tr class="table_style_content" align="center" onMouseMove="this.bgColor='F0F5F9'" onMouseOut="this.bgColor='#ffffff'">
					 <%-- 复合主键用“|”间隔开 --%>
                     <td>
                         <input type="checkbox" name="param._selectitem" value="<s:property value="prodid + '|' + ruleid"/>" onclick="checkOne();">
                     </td>
                     <td>
                     <s:if test="form.isEnabled=='isActive'">
                     ...
                     <s:else>
                     	<a href='<s:url action="pquantity_edit.do">
	                        <s:param name="param._pk" value="prodid + '|' + ruleid"/>
                     ...
                     <td>
	                     <j:code2Name definition="$FX_SMPBRAND" code="prodid" />
					 </td>
                     <td><s:i18n name="public">
						 <s:property value="%{getText('format.double',{incratio})}" />
						 </s:i18n></td>
