...
public class ActivelistsmpTaskBean extends BaseBatchTaskBean {

	public ActivelistsmpTaskBean() throws Exception {
		super.setBatchName("网点套卡激活统计批量导出");
		super.setOprtype("导出");
	...
	protected String doStart() {
		return "序号|分公司|服务销售中心|微区域|网点编码|网点名称|星级|品牌|商品种类|激活数量|激活率|上级渠道\r\n";
	}
	...
	public void doProcessFile(String fileInPath, String fileOutPath)
		...
		try {
			...
			DataPackage dp = snptbo.doQueryActiveSmpListBatchExp(params);
			...
			for (int i = 0; i < list.size(); i++) {
				...
				fos.write((i+1) + "|" + Code2NameUtils.code2Name("#CNTYCOMPANY", (vo.getCountyid() == null ? "" : vo.getCountyid()), user.getCityid())
					...
					+ "|" + (vo.getActrate() == null ? "" : numberFormat.format(vo.getActrate()*100)+"%")
