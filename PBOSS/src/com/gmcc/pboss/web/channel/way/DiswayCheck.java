...
public class DiswayCheck extends BaseCheckFormat {
...
// TODO Auto-generated method stub
		...
		// 检查列数
		if (items.length != 52) {
			throw new Exception("上传数据列数不对,应为51列");
		}
		...
		if (wayVO == null) {
			...
			}else if(delegate.doFindByPk(items[2].trim())==null)
			{
				throw new Exception("[上级渠道]在系统中不存在");
			}
			...
			// 合作商类型
			if (!StringUtils.isBlank(items[49])) {
				if (!checkDictitem(items[49],"CH_DISTYPE",user)) {
					throw new Exception("固定参数[合作商类型]的值不正确");
				}
			}
			// 星级
			if (!StringUtils.isBlank(items[50])) {
				if (!checkDictitem(items[50],"CH_STARLEVEL",user)) {
					throw new Exception("固定参数[星级]的值不正确");
				}
			}
		} else {
			// 渠道名称1
			...
			}else if(StringUtils.isNotBlank(items[2]) && delegate.doFindByPk(items[2].trim())==null)
			{
				throw new Exception("[上级渠道]在系统中不存在");
			}
		...
		// 负责人姓名14-------------------------------------------
		bchcontactVO = bchdelegate.doFindByPk(bchcontactVO.getWayid());
		...
		// 合同编码20---------------------------------------------------------------------
		waycompactVO = comdelegate.doFindByPk(waycompactVO.getWayid());
