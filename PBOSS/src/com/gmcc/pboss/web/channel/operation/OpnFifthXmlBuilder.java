...
public class OpnFifthXmlBuilder extends AbstractXmlBuilder {
	...
	public OpnFifthXmlBuilder(User user) {
		this.user = user;
	}

	public String getChildXml(XTreeNode xtreeNode) throws Exception {
		String nodeType = xtreeNode.getType();
		int curr = StringUtils.isEmpty(opntype) ? 6 : (new Integer(opntype)
				.intValue());
		if ("opn5".equals(nodeType)) {
