...
//    	os = new GZIPOutputStream(os); //采用压缩流传输
                ...
                while (itt.hasNext()) {
                    ...
                    for (int i = 0; i < map.size(); i++) {
                        ...
                        if (EXPRESSION.equals(propertyFormat.format)) { //处理两值相加的表达式
                            String value = expressionProc2(obs, vo,
...
//				 init
    ...
    protected String formatData(PropertyFormat propertyFormat, Object value)
        ...
        } else if (propertyFormat.format.equals("RATE")) {
        	 NumberFormat numberFormat = new DecimalFormat(
                     propertyFormat.formatType);
             double strdouble;
     ...
     * @return String
     */
    private String expressionProc(Object[] property, Class[] vo,
    ...
    /**
     * 处理表达式的函数 现只支持 简单的四则运算（修改expressionProc(Object[] property, Class[] vo,
     * String expression, String type) 方法,
     * 增加如果表达式中有数值常量则不从实体类的查询，真接使用此数值常量做计算,去掉需要DOUBLE类型的限制
     * @param property Object[]
     * @param vo Class[]
     * @param expression String 
     * @param type String
     * @return String
     */
    private String expressionProc2(Object[] property, Class[] vo,
                                  String expression, String type) {
        int pos = 0;        
        List propertys = new ArrayList();
        List oprs = new ArrayList(0);       
        splitOpr( expression, propertys, oprs );
	        ...
	        Double valueD = 0.0;
	        if(isNumeric(propertys.get(0).toString(),false)){
	        	valueD = Double.valueOf( propertys.get(0).toString());
	        }else{
	        	Object obj = getValue(property, vo,
		                getPropertyFormat( propertys.get(0).toString() ).propertyId, propertys.get(0).toString());
	        ...
	        for( int i=0;i<oprs.size();i++ ){
	        	...
	        	if(isNumeric(propertys.get(i+1).toString(),false)){
	 ...
	 * 验证指定串是否数值
	 * @param src 待难证串
	 * @param isSign 是否可带正负号 是：TRUE 否：FALSE
	 * @return
	 */
	private static boolean isNumeric(String src, boolean isSign) {
		if (isSign)
