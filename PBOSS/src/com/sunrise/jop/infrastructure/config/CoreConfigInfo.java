...
public class CoreConfigInfo {
	...
	//登录是否分流访问鉴权集群（yes/no）
	public static boolean LOGIN_DISTRIBUTION=true;
	/**
	 * ------------------------------------------------------------------------
	 */
	...
	static {
		try {
			...
			LOGIN_DISTRIBUTION = rs.getString("login.distribution").equalsIgnoreCase("yes");
			...
			LOGIN_VERSION=rs.getString("login.version");
