...
public class StringUtil {
	...
	private static String packageName = "com.huawei.cbs.cbsinterface";// cbs
																		// route
	...
	private static DateFormat dateTemplate14 = new SimpleDateFormat(
			"yyyyMMddHHmmss");
	private static DateFormat dateTemplate19 = new SimpleDateFormat(
			"yyyy-MM-dd HH:mm:ss");
	...
	public static boolean checkMonth(String month){
		if (!checkNullAndBlank(month)) {
			return false;
		}
		// 正则匹配式
		String matchCase = "[1-9][0-9]{3}((0[0-9])|(1[0-2]))";
		return month.matches(matchCase);
	}
	...
	public static java.util.Date changeStrToDate8(String item, String datetime)
		...
		if (checkDateTime8(datetime)) {
			...
			throw new Exception("["+item+"]"+datetime + ":日期格式不符合YYYYMMDD");
		}
	}
	...
	public static java.util.Date changeStrToDate14(String item, String datetime)
		...
		if (checkDateTime14(datetime)) {
			...
			throw new Exception(item + " (" + datetime
					+ ") :日期格式不符合YYYYMMDDHHMMSS");
		}
	}
	...
	public static String formatTimeType14(String str) throws Exception {
		...
		if (str.length() != 14) {
			...
			datestr.append(str.substring(0, 4)).append("-").append(
					str.substring(4, 6)).append("-")
					.append(str.substring(6, 8)).append(" ").append(
	...
	public static Long formatLong(String item, String str) throws Exception {
		// CRM会传"null"这样字符过来导致转换失败。修改兼容CRM的这种错误
	...
	public static Double formatDouble(String item, String str) throws Exception {
		if (null == str || "".equals(str) || "null".equalsIgnoreCase(str))
			return null;
		try {
	...
	public static Byte formatByte(String item, String str) throws Exception {
		if (null == str || "".equals(str) || "null".equalsIgnoreCase(str))
			return null;
		try {
			return Byte.parseByte(str);
		} catch (Exception e) {
			throw new Exception(item + "转换失败。" + str + " 不是合法数字，或超过位数限制。");
		}
	}

	public static Integer formatInteger(String item, String str)throws Exception {
		if (null == str || "".equals(str) || "null".equalsIgnoreCase(str))
			return null;
		try {
	...
	public static Boolean formatBoolean(String item, String str)throws Exception {
		if(null == str || "".equals(str) || "null".equalsIgnoreCase(str))
			return null;
		if(str.trim().length() > 1) {
			if(str.trim().toLowerCase().equals("true")) {
				...
				throw new Exception(item + "转换失败。" + str + " 不能转换成有效的逻辑值。");
			}
		}else{
	...
	public static Short formatShort(String item, String str) throws Exception {
		if (null == str || "".equals(str) || "null".equalsIgnoreCase(str))
			return null;
		try {
	...
	public static String complexObject2Str(Object vo) {
		...
		try {
			...
			if (voClass.getName().endsWith("req")) {// RequestHeader
				buff.append("<MsgReqHeader>");// MessageHeader
			} else {
					...
					try {
						...
						if (StringUtil.checkNullAndBlank(value)) {
							if (fields[i].getType().getName().endsWith("Date")) {// 日期要进行转换
								value = StringUtil.changeAreaDateToStr19(value);// 此处需要改造成协议适应的格式？？？？
							}
							// <element>string</element>
							buff.append("<").append(fields[i].getName())
									.append(">");
							buff.append(value);
							buff.append("</").append(fields[i].getName())
									.append(">");
						...
						log.info(voClass.getSimpleName() + "对象,读取属性("
								+ fields[i].getName() + ")出错.");
					}
				} else {
					// 递归解释复杂属性对象
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static Object paraseElement(Element element, String className)
			throws Exception {
		Object object = null;
		List list = null;
		...
		try {
			...
			for (Iterator<Element> it = element.elementIterator(); it.hasNext();) {
				...
				if (!tmpIndexStr.toUpperCase().equals(tmpIndexStr)) {
					attrName = elementName.substring(0, 1).toLowerCase()
				...
				if (e.isTextOnly()) {// 简单对象:字符串
					...
					// tmpClassName = paraListPackageName+elementName;
						...
						if (objectName.endsWith("List")) {// AcctList ,
															// ExtendObjects
							// List对象设置属性值要特殊处理
							listName = attrName;
							list.add(obj);
	...
	public static String getProcessCode(Msgreqheader reqHeader) {
		return "[" + reqHeader.getProcessCode() + "-" + reqHeader.getReqSeq().trim()+ "]---->";
	}
