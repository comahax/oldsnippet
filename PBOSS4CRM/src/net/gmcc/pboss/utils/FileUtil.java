...
//import net.gmcc.pboss.common.control.ParamTask;
...
public class FileUtil {
	...
	public static void writeFile(String path, String content, boolean append){
		try {
			...
			e.printStackTrace();
			log.info(path+" 写文件出错."+e.getMessage());
		}
	}
	...
	public static boolean deleteFile(String path) {
		try {
			...
			log.info(path + " 删除文件出错。" + e.getMessage());
			return false;
		}
		return true;
	}
...
//		return table+
//		       ParamsUtil.getRegionLetterMap().get(areaNo)+
...
//					e.printStackTrace();
					log.info(voClass.getSimpleName()+ "对象,读取属性("+tmpfields[i]+")出错.");
				}
			}
		} catch (Exception e) {
//			e.printStackTrace();
			log.info( "不能遍历VO:"+vo.getClass().getName()+", "+e.getMessage());
		}
...
//			BufferedReader fileReader = new BufferedReader(new InputStreamReader(new FileInputStream(file)));//默认字符编码:GBK
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			log.info(file.getName()+"读取文件出错."+e.getMessage());
		}
...
//		String path = ParamTask.class.getResource("").getPath();
...
//				log.info("开始读取文件 :" + path + propertiesName);
//				properties.load(new FileReader(path + propertiesName));
//			} catch (Exception e) {
//				// TODO Auto-generated catch block
//				e.printStackTrace();
//				log.info("读取ds.properties文件出错."+e.getMessage());
//			}
//		} else {
//			log.info("路径为:" + path);
//			log.warn("找不到 WEB-INF 目录,不会加载 "+propertiesName+" 文件.");
//		}
//		
//		return properties;
//	}
