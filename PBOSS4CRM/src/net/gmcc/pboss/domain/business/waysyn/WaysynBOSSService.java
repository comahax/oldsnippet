...
import net.gmcc.pboss.common.dynamicds.DynamicSource;
...
@Service
public class WaysynBOSSService extends BaseCrmService {
	private static final Logger log = Logger.getLogger(WaysynBOSSService.class);
	...
	public void doBusiness(Waysynreq request, Msgrspheader rspheader)throws Exception{		
		try{		
			...
			if(oprtype!=null && OprType.CREATE.equals(oprtype)){
				...
				DynamicSource ds = (DynamicSource)ServiceUtil.ac.getBean("dataSource");					
				dao.getCurrentSession().reconnect(ds.getConnection());
				...
				ds = (DynamicSource)ServiceUtil.ac.getBean("dataSource");	
				dao.getCurrentSession().reconnect(ds.getConnection());
